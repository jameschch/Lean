...
namespace QuantConnect.Orders.Fees
{
    ...
    public class OKCoinFeeModel : IFeeModel
    {
        ...
        public decimal GetOrderFee(Securities.Security security, Order order)
        {
            ...
            if (order.Type == OrderType.Limit && ((((LimitOrder)order).LimitPrice > security.BidPrice && order.Direction == OrderDirection.Sell) ||
            (((LimitOrder)order).LimitPrice < security.AskPrice && order.Direction == OrderDirection.Buy)))
            {
                //0% maker fee
                return 0;
            }
