...
namespace QuantConnect.Brokerages.OKCoin
{
    ...
    public partial class OKCoinBrokerage : BitfinexWebsocketsBrokerage
    {
        ...
        public override List<Holding> GetAccountHoldings()
        {
            ...
            foreach (string okcoinSymbol in new[] { "btc_usd", "ltc_usd" })
            {

                foreach (OKCoinContractType contractType in new[] { OKCoinContractType.next_week, OKCoinContractType.this_week, OKCoinContractType.quarter })
                {
                    ...
                    string response = _httpClient.Post("future_position.do", parameters);
                    var raw = JsonConvert.DeserializeObject<dynamic>(response, settings);

                    if (raw != null && raw.holding != null)
                    {
                        foreach (var item in raw.holding)
                        {
                            string itemSymbol = (string)item.symbol;

                            list.Add(new Holding
                            {
                                AveragePrice = item.buy_price_avg > 0 ? (decimal)item.buy_price_avg : (decimal)item.sell_price_avg,
                                CurrencySymbol = itemSymbol.Substring(0, 3).ToUpper(),
                                Quantity = (item.buy_amount > 0 ? (decimal)item.buy_amount : -(decimal)item.sell_amount),
                                Symbol = Symbol.Create(itemSymbol.ToUpper().Replace("_", ""), SecurityType.Forex, Market.OKCoin.ToString()),
